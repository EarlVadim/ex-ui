...
class Inbound extends XrayCommonClass {
    ...
    genVLESSLink(address = this.extlisten, port=this.extport, forceTls, remark='', clientId, flow) {
        const uuid = clientId;
    ...
    genTrojanLink(address = this.extlisten, port=this.extport, forceTls, remark = '', clientPassword) {
        const security = forceTls == 'same' ? this.stream.security : forceTls;
        const type = this.stream.network;
        const params = new Map();
        params.set("type", this.stream.network);
    ...
    genLink(address = this.extlisten, port=this.extport, forceTls='same', remark='', client) {
        switch (this.protocol) {
    ...
    genAllLinks(remark='', remarkModel = '-ieo', client){
       ...
       // let addr = !ObjectUtil.isEmpty(this.listen) && this.listen !== "0.0.0.0" ? this.listen : location.hostname;
	    let addr = this.extlisten;
        let port = this.extport;
